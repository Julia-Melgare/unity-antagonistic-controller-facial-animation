...
public class lb_Bird : MonoBehaviour {
	...
	void OnEnable () {
		if (useRandomSeed) randomGenerator = new System.Random(randomSeed);
	...
	void OnGroundBehaviors(){
		...
		if(idle){
			...
			if (randomGenerator.NextDouble() < Time.deltaTime*.33){
			...
			if (randomGenerator.NextDouble() < Time.deltaTime*.1){
				FlyAway ();
			}
		}
	}
	...
	void Flee(){
		if(!dead){
			...
			farAwayTarget += new Vector3(((float)randomGenerator.NextDouble()*200-100)*controller.birdScale,10*controller.birdScale,((float)randomGenerator.NextDouble()*200-100)*controller.birdScale);
